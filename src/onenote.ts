 ...
 * the token, create the Graph client, and apply error handling.
 */
export class OneNoteClient {
  ...
  constructor(token: string) {
    this.token = token;
    this.client = createGraphClient(token);
  }

  /**
   * Build a client from the cached token on disk / env var.
   * Throws if no token is available.
   */
  static fromStoredToken(): OneNoteClient {
    const token = loadToken();
    if (!token) {
      throw new Error(
        "Access token not found. Run 'npm run auth' (or the 'authenticate' MCP tool) first.",
      );
    }
    return new OneNoteClient(token);
  }
  ...
  async findPage(query: string): Promise<OnenotePage | null> {
    ...
    const byTitle = pages.find(
      (p) => p.title && p.title.toLowerCase().includes(lower),
    );
    if (byTitle) return byTitle;

    // Partial ID match
    const byPartialId = pages.find(
      (p) => p.id?.includes(query) || query.includes(p.id ?? ''),
    );
